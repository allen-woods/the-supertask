...
openssl_create_ld_library_path_script () {
  [ ! -f $(pwd)/openssl.sh ] && \
  echo -e '#!/bin/sh \nenv LD_LIBRARY_PATH='"${HOME}/local/lib ${HOME}/local/bin"'/openssl "$@"' > $(pwd)/openssl.sh
}
openssl_change_ownership_of_ld_library_path_script () {
  chown root:root $(pwd)/openssl.sh
}
openssl_make_ld_library_path_script_executable () {
  chmod 0755 $(pwd)/openssl.sh
}
openssl_change_to_usr_bin_dir () {
  cd /usr/bin
}
openssl_create_dynamic_link_in_usr_bin () {
  ln -s $HOME/local/bin/openssl.sh $(pwd)/aes-wrap
}
openssl_confirm_app_will_run () {
  if [ $1 -gt 0 ]; then
...
# NOTE: The following functions are provided here for completeness only.
openssl_remove_build_dir () {
  rm -rf $HOME/src
}
